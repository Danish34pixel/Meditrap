...
class ImageUploadService {
  ...
  async uploadToServer(file, uploadUrl, additionalData = {}) {
    try {
...
export const useImageUpload = () => {
  ...
  const selectAndUploadImage = async (uploadUrl, additionalData = {}) => {
    try {
      // Process the image (validate and store locally)
      const processResult = await imageService.processImage();

      if (!processResult.success) {
        Alert.alert('Error', processResult.message);
        return processResult;
      }

      // Upload to server
      const uploadResult = await imageService.uploadToServer(
        processResult.file,
        uploadUrl,
        additionalData,
      );
      ...
      if (!uploadResult.success) {
        Alert.alert('Upload Error', uploadResult.message);
        // Clean up local file if upload failed
        await imageService.deleteFile(processResult.file.path);
        return uploadResult;
      }
